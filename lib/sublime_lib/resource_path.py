...
class ResourceRoot(metaclass=ABCMeta):
    ...
    def file_to_resource_path(self, file_path: Union[Path, str]) -> Optional['ResourcePath']:
        ...
        if not file_path.is_absolute():
            raise ValueError(
                "Cannot convert a relative file path to a resource path.")
...
class InstalledResourceRoot(ResourceRoot):
    ...
    def _package_resource_path(self, package: str, *rest: str) -> 'ResourcePath':
        package_path = (self.resource_root /
                        package).remove_suffix('.sublime-package')
...
def get_roots() -> List[ResourceRoot]:
    ...
    if _ROOTS is None:
        _ROOTS = [
            ...
            InstalledResourceRoot(
                'Packages', sublime.installed_packages_path()),
            InstalledResourceRoot('Packages', Path(
                sublime.executable_path()).parent / 'Packages'),
        ]
    return _ROOTS
...
class ResourcePath():
    ...
    def from_file_path(cls, file_path: Union[Path, str]) -> 'ResourcePath':
        ...
        file_path = wrap_path(file_path)
        candidates = (root.file_to_resource_path(file_path)
        ...
        if path:
            ...
            raise ValueError(
                "Path {!r} does not correspond to any resource path.".format(
                    file_path)
            )
    ...
    def relative_to(self, *other: object) -> Tuple[str, ...]:
        ...
        if other_path.parts == self._parts[:other_len]:
            ...
            raise ValueError(
                "{!s} does not start with {!s}".format(self, other_path))
    ...
    def remove_suffix(
        ...
        if new_name is not None:
            ...
            raise ValueError(
                'Cannot remove suffix {!r} from {!r}.'.format(suffix, self))
        else:
            return self
    ...
    def file_path(self) -> Path:
        ...
        raise ValueError(
            "Can't find a filesystem path for {!r}.".format(self.root)) from None
